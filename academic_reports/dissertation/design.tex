...
what: energy-efficient sensing library
idea: leverages the relation between sensors: GPS and accelerometer
		detecting movement (ref), if not moving then switch off GPS
features summary -> requirements/objectives:
	moving detection algorithm (ref)
	energy efficiency usage of that algorithm + it depending on the battery life (ref)
	library must have specific architecture to be plugged into ESM (ref)

\subsubsection{Moving detection algorithm}
\label{s:design:locy:moving}
algorithm for movement detection:
-simplification: total magnitude of accelerometer coordinates		
-idea: peak when walking
	-graph: walking and not walking
-formalize: standard deviation
-problem: threshold \& sliding window
-solution:
	-trial and error against different data sets
	-2,5 seconds sliding window
	-30 as a threshold
		show: the same graph walking and not walking but standard deviation and threshold
	-three sliding windows to make sure
	-such algorithm should be universal
		(not user dependent)
...
what: duty-cycling
	...
	-listen \& sleep -> conserve an energy
reason: user's does not change its activity so often
	but what ratio is sufficient?
advancement:
	dependent it on battery life
	battery life low, sample less
		show table

\subsection{Software architecture}
\label{s:design:locy:architecture}
problem: plug to existed programs
	don't want to force developers to change much things
solution: looks like LocationManager provided by Android
	one class is visible LoctManager which has LocationManagerAPI
	effectively for ESM it requires changing one line:
		show example
